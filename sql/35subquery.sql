...
# subquery는 SELECT, FROM, WHERE 에 작성 가능
...
SELECT CustomerID, CustomerName, (SELECT COUNT(OrderID) FROM Orders o WHERE o.CustomerID) 건
FROM Customers c
ORDER BY 건 DESC;
...
# subquery :
# 상관 서브쿼리 : 외부 쿼리의 값을 내부 쿼리에서 사용함
# 비상관 서브쿼리 : 외부 쿼리의 값을 내부에서 사용하지 않음
